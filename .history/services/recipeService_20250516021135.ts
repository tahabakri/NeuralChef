...
export async function generateRecipe(  ingredients: string[],   dietaryPreference: string = 'all'): Promise<Recipe> {  try {    // Validate input    if (!ingredients || ingredients.length === 0) {      throw { type: 'validation' as RecipeErrorType, message: 'Please provide at least one ingredient' };    }        // Simulate API call delay    await new Promise(resolve => setTimeout(resolve, 1500)); // Reduced delay slightly        // Filter recipes by dietary preference if specified    const filteredRecipes = dietaryPreference === 'all'       ? mockRecipes       : mockRecipes.filter(recipe => {          // This is a simplified filtering logic - in a real app, you would have           // more sophisticated logic based on ingredients and recipe properties          switch (dietaryPreference) {            case 'vegetarian':              return !recipe.ingredients.some(i =>                 i.toLowerCase().includes('meat') ||                 i.toLowerCase().includes('chicken') ||                 i.toLowerCase().includes('pork') ||                 i.toLowerCase().includes('beef')              );            case 'vegan':              return !recipe.ingredients.some(i =>                 i.toLowerCase().includes('meat') ||                 i.toLowerCase().includes('egg') ||                 i.toLowerCase().includes('milk') ||                 i.toLowerCase().includes('cheese') ||                 i.toLowerCase().includes('chicken') ||                 i.toLowerCase().includes('pork') ||                 i.toLowerCase().includes('beef')              );            case 'gluten-free':              return !recipe.ingredients.some(i =>                 i.toLowerCase().includes('flour') ||                 i.toLowerCase().includes('pasta') ||                 i.toLowerCase().includes('bread') ||                 i.toLowerCase().includes('wheat')              );            default:              return true;          }        });        // If no recipes match the dietary preference, fall back to all recipes    const recipesToChooseFrom = filteredRecipes.length > 0 ? filteredRecipes : mockRecipes;        // Select a random recipe from the filtered list    const randomIndex = Math.floor(Math.random() * recipesToChooseFrom.length);    let selectedRecipe = { ...recipesToChooseFrom[randomIndex] }; // Create a copy to modify
