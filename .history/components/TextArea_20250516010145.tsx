...
const TextArea: React.FC<TextAreaProps> = ({
  ...
  const recordingAnimatedStyle = useAnimatedStyle(() => {
    return {
      ...
      transform: [
        { scale: isRecording ? recordingAnimation.value : (1 + recordingAnimation.value * 0.1) }
      ]
    };
  });
  ...
  const startVoiceRecording = () => {
    ...
    // Animate the recording button - add pulsing effect
    recordingAnimation.value = withTiming(1, {
      duration: 300,
      easing: Easing.bezier(0.25, 0.1, 0.25, 1),
    });
    
    // Create pulsing animation for the mic button
    ...
    // Apply the pulse animation to the scale
    ...
    setTimeout(() => {
      ...
      cancelAnimation(recordingAnimation);
