...
// Try to import toast - will use CustomToast if this fails
let Toast: any;
try {
  Toast = require('react-native-toast-message');
} catch (error) {
  console.warn('react-native-toast-message not available, using fallback');
  Toast = null;
}
...
const TextArea: React.FC<TextAreaProps> = ({
  ...
  const startVoiceRecording = () => {
    ...
    const startRippleAnimation = () => {
      ...
      rippleScale.value = withTiming(1.8, { duration: 1000 });
    ...
    setTimeout(() => {
      ...
      if (onVoiceInput) {
        ...
        // Show confirmation toast with slight delay for better UX
        if (addedItems.length > 0) {
          const toastMessage = `Added: ${addedItems.join(', ')}`;
          ...
          setTimeout(() => {
            if (Toast) {
              ...
              try {
                ...
                // Fall back to custom toast
                setCustomToastMessage(toastMessage);
    ...
    return (
      <Animated.View style={animatedStyle}>
        <TouchableOpacity
          ...
          accessibilityLabel={`Ingredient: ${tag.text}, tap to remove`}
          accessibilityHint="Double tap to remove this ingredient"
        >
          <Text style={[
            styles.tagText,
  ...
  return (
    <View style={[styles.container, style]}>
      {/* Tags */}
      <View style={styles.tagsContainer} accessibilityLabel="Ingredient tags list">
        <FlatList
          data={tags}
          horizontal
          ...
          ListEmptyComponent={() => (
            <Text style={styles.placeholderText} accessibilityLabel="No ingredients added yet">No ingredients added yet</Text>
          )}
        />
      </View>
      ...
      {showSuggestions && (
        <Animated.View 
          ...
          accessibilityLabel="Ingredient suggestions"
        >
