...
const STEP_IMAGES = [
  ...
  'https://images.unsplash.com/photo-1530047139081-81614f3e0d3e?q=80&w=2940&auto=format&fit=crop',
];
...
export default function RecipeStep({
  ...
  const handlePress = () => {
    ...
    // Animate the step when marked as done
    if (newIsDone) {
      // Haptic feedback
      if (Platform.OS !== 'web') {
  ...
  return (
    <Animated.View style={[styles.container, isDone && styles.containerDone, animatedContainerStyle]}>
      ...
      {/* Toast container for timer notifications */}
      <Toast />
    </Animated.View>
  );
}
