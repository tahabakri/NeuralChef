...
export default function TabOneScreen() {
  ...
  // Create shared values for each card at the component level
  const cardScales = popularCombinations.map(() => useSharedValue(1));
  
  // Create animated styles for each card at the component level
  const cardAnimatedStyles = popularCombinations.map((_, index) => 
    useAnimatedStyle(() => ({
  ...
  const handleCombinationPress = (index: number) => {
    // Animate the card scale
    cardScales[index].value = withSpring(0.95, {}, () => {
      cardScales[index].value = withSpring(1);
    });
    ...
    const combination = popularCombinations[index];
  ...
  const handleGenerateRecipes = () => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      Alert.alert('Recipes Generated', `Found recipes with: ${tags.map(tag => tag.text).join(', ')}`);
    }, 1500);
  };
      ...
      >
        ...
        <View style={styles.popularSection}>
          ...
          <FlatList
            ...
            renderItem={({ item, index }) => (
              <Animated.View style={cardAnimatedStyles[index]}>
                <TouchableOpacity
                  style={styles.combinationCard}
                  activeOpacity={0.8}
                  onPress={() => handleCombinationPress(index)}
                >
                  <ImageBackground
        ...
        <View style={styles.recentSection}>
          ...
          <View style={styles.recentItems}>
            {/* This would be populated with the user's recent ingredients/recipes */}
            <Text style={styles.emptyStateText}>
              Your recently used ingredients and recipes will appear here.
            </Text>
          </View>
        </View>
      </ScrollView>
