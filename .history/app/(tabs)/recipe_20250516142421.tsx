import React, { useState, useEffect, useRef } from 'react';import { StyleSheet, Text, View, ScrollView, Platform, Pressable, Alert, Share, Animated, FlatList } from 'react-native';import { useRouter } from 'expo-router';import * as Haptics from 'expo-haptics';import { Image } from 'expo-image';import { ChevronLeft, Clock, Users, Flame, Heart, Share2, TimerOff, Bookmark, BarChart, Star, ChevronDown, ChevronUp, Coffee, CircleCheck, ChefHat, HelpCircle, Sparkles } from 'lucide-react-native';import * as Clipboard from 'expo-clipboard';import Toast from 'react-native-toast-message';import Reanimated, { useSharedValue, useAnimatedStyle, withRepeat, withTiming, Easing, withSequence, interpolate, useAnimatedScrollHandler } from 'react-native-reanimated';import { LinearGradient } from 'expo-linear-gradient';import Button from '@/components/Button';import PulseButton from '@/components/PulseButton';import EmptyStateAnimation from '@/components/EmptyStateAnimation';import TutorialModal from '@/components/TutorialModal';import RecipeStep from '@/components/RecipeStep';import IngredientsList from '@/components/IngredientList';import LoadingOverlay from '@/components/LoadingOverlay';import CategoryTag from '@/components/CategoryTag';import TagSelector from '@/components/TagSelector';import colors from '@/constants/colors';import { useRecipeStore, getUserFriendlyErrorMessage } from '@/stores/recipeStore';import { useSavedRecipesStore } from '@/stores/savedRecipesStore';import { useUserStore } from '@/stores/userStore';
          ...
          <View style={styles.section}>
            ...
            <Pressable onPress={handleCopyIngredients} style={styles.copyButton}>
              <Text style={styles.copyButtonText}>Copy Ingredients</Text>
                    ...
                    {/* Directions Section */}          <View style={styles.section}>            <Text style={styles.sectionTitle}>Directions</Text>            {recipe.steps.map((step, index) => (              <RecipeStep                key={index}                number={index + 1}                instruction={step.instruction}                imageUrl={step.imageUrl}              />            ))}          </View>                    {/* Tags Section */}          <View style={styles.section}>            <TagSelector              recipeTags={recipeTags}              onTagsChange={handleTagsChange}              recipeTitle={recipe.title}              ingredients={recipe.ingredients}            />          </View>
...
const styles = StyleSheet.create({
  ...
  emptyImageContainer: {
    width: 200,
    height: 200,
    marginBottom: 36, // Increased for better vertical spacing
    justifyContent: 'center',
    alignItems: 'center',
  },
  emptyTitle: {
    fontSize: 24, // Increased from previous size
  ...
  emptyMessage: {
    fontSize: 16,
    color: '#666', // Slightly lighter gray as requested
    marginBottom: 36, // Increased margin
    textAlign: 'center',
    lineHeight: 24,
  },
  emptyButton: {
    minWidth: 220, // Wider button
    marginBottom: 24, // Space for quick start button
  },
  quickStartButton: {
    flexDirection: 'row',
    alignItems: 'center',
