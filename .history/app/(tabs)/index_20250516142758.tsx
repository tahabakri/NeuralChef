...
import React, { useState, useRef, useEffect } from 'react';import {  View,  Text,  StyleSheet,  ScrollView,  Image,  TouchableOpacity,  FlatList,  Dimensions,  Platform,  ActivityIndicator,  ImageBackground,  StatusBar,  Alert,  Pressable,  Modal,} from 'react-native';import { Image as ExpoImage } from 'expo-image';import { LinearGradient } from 'expo-linear-gradient';import { ChevronRight, Sparkles, History, Trash2, ShoppingBasket, Camera, Tag, Filter, X } from 'lucide-react-native';import * as Haptics from 'expo-haptics';import { useRouter } from 'expo-router';import * as ImagePicker from 'expo-image-picker';import Animated, {   useSharedValue,   useAnimatedStyle,   withTiming,   withSequence,  Easing} from 'react-native-reanimated';import TextArea from '../../components/TextArea';import Button from '../../components/Button';import Card from '../../components/Card';import PopularCombinationCard from '../../components/PopularCombinationCard';import FilterDropdown from '../../components/FilterDropdown';import TagFilter from '../../components/TagFilter';import CategoryTag from '../../components/CategoryTag';import { popularCombinations, validateIngredient } from '../../constants/ingredients';import { useRecipeHistoryStore } from '../../stores/recipeHistoryStore';import { useRecipeStore } from '../../stores/recipeStore';import { usePreferencesStore } from '../../stores/preferencesStore';import { useSavedRecipesStore } from '../../stores/savedRecipesStore';import { Recipe, generateRecipe, RecipeServiceError } from '../../services/recipeService';import colors from '../../constants/colors';import { extractTextFromImage } from '../../services/imageService';
...
export default function TabOneScreen() {  const [inputText, setInputText] = useState('');  const [tags, setTags] = useState<Tag[]>([]);  const [isLoading, setIsLoading] = useState(false);  const [recentIngredients, setRecentIngredients] = useState<string[]>([]);  const [ocrIngredients, setOcrIngredients] = useState<string[]>([]);  const [isOcrLoading, setIsOcrLoading] = useState(false);  const scrollRef = useRef<ScrollView>(null);  const router = useRouter();  const { history: recipeHistory, clearHistory, addToHistory, getHistoryRecipesByTags } = useRecipeHistoryStore();  const { setRecipe, setError, clearError } = useRecipeStore();  const { dietaryPreference, setDietaryPreference } = usePreferencesStore();  const { savedRecipes, getRecipesByTags } = useSavedRecipesStore();    // Tag filtering state  const [selectedTags, setSelectedTags] = useState<string[]>([]);  const [showTagFilter, setShowTagFilter] = useState(false);  const [filteredRecipes, setFilteredRecipes] = useState<Recipe[]>([]);  const [filteredHistory, setFilteredHistory] = useState<Recipe[]>([]);
  
    // Load recent ingredients from AsyncStorage on component mount  useEffect(() => {    const loadRecentIngredients = async () => {      try {        // You would typically load these from AsyncStorage        // This is a placeholder - implement actual storage in recipeHistoryStore        setRecentIngredients(['Chicken', 'Pasta', 'Tomatoes', 'Basil', 'Garlic']);      } catch (error) {        console.error('Failed to load recent ingredients:', error);      }    };        loadRecentIngredients();  }, []);    // Filter recipes when selectedTags change  useEffect(() => {    if (selectedTags.length === 0) {      // No filters, show all recipes      setFilteredRecipes(savedRecipes);      setFilteredHistory(recipeHistory);    } else {      // Apply tag filtering      setFilteredRecipes(getRecipesByTags(selectedTags));      setFilteredHistory(getHistoryRecipesByTags(selectedTags));    }  }, [selectedTags, savedRecipes, recipeHistory, getRecipesByTags, getHistoryRecipesByTags]);
    ...
    const handleToggleTagFilter = () => {    setShowTagFilter(!showTagFilter);  };    const handleTagsChange = (tags: string[]) => {    setSelectedTags(tags);  };    return (    <View style={styles.container}>      <StatusBar barStyle="dark-content" />            {/* Gradient background */}      <LinearGradient        colors={[colors.backgroundGradientStart, colors.backgroundGradientEnd]}        style={styles.gradientBackground}      />            {/* Tag Filter Modal */}      <Modal        animationType="slide"        transparent={true}        visible={showTagFilter}        onRequestClose={() => setShowTagFilter(false)}      >        <View style={styles.modalContainer}>          <View style={styles.modalContent}>            <View style={styles.modalHeader}>              <Text style={styles.modalTitle}>Filter Recipes by Tags</Text>              <TouchableOpacity onPress={() => setShowTagFilter(false)}>                <X size={24} color="#000000" />              </TouchableOpacity>            </View>                        <TagFilter               selectedTags={selectedTags}              onTagsChange={handleTagsChange}            />                        <Button              title="Apply Filters"              onPress={() => setShowTagFilter(false)}              style={styles.applyButton}            />          </View>        </View>      </Modal>            <ScrollView        ref={scrollRef}        style={styles.scrollView}        contentContainerStyle={styles.contentContainer}        showsVerticalScrollIndicator={false}      >
        ...
        <View style={styles.inputSection}>
          <TextArea
            ...
            initialOcrIngredients={ocrIngredients}
            isLoadingOcr={isOcrLoading}
          />
